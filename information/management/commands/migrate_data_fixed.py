...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write("🔄 Starting data migration from SQLite to MySQL...")
        ...
        try:
            ...
            for project in projects:
                try:
                    ...
                    self.stdout.write(f"✅ Migrated project: {project_data['title']}")
            ...
            for competence in competences:
                try:
                    ...
                    self.stdout.write(f"✅ Migrated competence: {competence_data['title']}")
                except Exception as e:
            ...
            try:
                ...
                for testimonial in testimonials:
                    try:
                        ...
                        self.stdout.write(f"✅ Migrated testimonial: {testimonial_data['name']}")
                    except Exception as e:
                ...
                self.stdout.write(self.style.WARNING(f"⚠️ No testimonials table found or error: {e}"))
            
            # 4. Migrate Information
            self.stdout.write("ℹ️ Migrating Information...")
            ...
            for info in info_records:
                try:
                    ...
                    self.stdout.write(f"✅ Migrated information: {info_data['name_complete']}")
                except Exception as e:
                    self.stdout.write(self.style.WARNING(f"⚠️ Error migrating information {info[0]}: {e}"))
            
            self.stdout.write(self.style.SUCCESS(f"✅ Migrated {len(info_records)} information records"))
            ...
            try:
                ...
                self.stdout.write(self.style.WARNING(f"⚠️ Error creating admin user: {e}"))
            
            # 6. Create default client user if it doesn't exist
            self.stdout.write("👤 Creating default client user...")
            try:
                ...
                if created:
                    ...
                    client_user.save()
                    self.stdout.write(self.style.SUCCESS("✅ Created default client user (client@example.com/client123)"))
                else:
                    self.stdout.write("✅ Client user already exists")
            except Exception as e:
                self.stdout.write(self.style.WARNING(f"⚠️ Error creating client user: {e}"))
